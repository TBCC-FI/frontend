...
/**
 * Fetch token analytics data for main page using the API
 * @returns
 */
export const fetchAnalyticsData = async (): Promise<ApiResponseTokenAnalyticsData> => {
  const requestPath = `${CORS_URL}/v2/cryptocurrency/quotes/latest?id=8487&aux=volume_30d,total_supply`;

  const res = await fetch(requestPath, {
    headers: {
      'X-CMC_PRO_API_KEY': 'b010f257-bbf6-4703-8b03-b58c9ae3647c',
  ...
  console.error(`API: Failed to fetch token analytics data`, res.statusText)
  return {
    data: {
      '8487': {
        quote: {
