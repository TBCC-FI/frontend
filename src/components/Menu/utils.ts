...
export const getActiveMenuItem = ({ pathname, menuConfig }: { pathname: string; menuConfig: ConfigMenuItemsType[] }) =>
  menuConfig.find((menuItem) => menuItem.href.includes(pathname) || getActiveSubMenuItem({ menuItem, pathname }))

export const getActiveTitleItem = ({ pathname, menuConfig }: { pathname: string; menuConfig: ConfigMenuItemsType[] }) => {
  const submit = menuConfig.find((menuItem) => menuItem.href.includes(pathname))
  return submit || getActiveSubTitleItem({ pathname, menuConfig })
}
...
export const getActiveSubTitleItem = ({ pathname, menuConfig }: { pathname: string; menuConfig: ConfigMenuItemsType[] }) => {
  const activeSubMenuItems = menuConfig.filter((menuItem) => pathname.startsWith(menuItem.href)) ?? []
  ...
  // Pathname includes multiple sub menu item hrefs - find the most specific match
  const specificMatch = activeSubMenuItems.sort(
    (subMenuItem1, subMenuItem2) => subMenuItem2.href.length - subMenuItem1.href.length,
  )[0]
